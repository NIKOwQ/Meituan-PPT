// Offline presentation adapter. All UI/rendering code below is the real frontend.
...
const state = {
  ...
  replayTurn: 0,
  replayEvalIndex: 0,
  replayMaxBatchSeen: 0,
  replayViewBatchIndex: 0,
  replayFollowLatest: true,
  replayInspecting: false,
};
...
batchPrev.addEventListener("click", () => {
  if (state.replayTimer) {
    state.replayInspecting = false;
    state.currentEvidenceKey = "";
    state.replayFollowLatest = false;
    state.replayViewBatchIndex = Math.max(0, getReplayDisplayBatchIndex() - 1);
    renderReplayFrame({ advance: false });
    return;
  }
...
batchNext.addEventListener("click", () => {
  if (state.replayTimer) {
    state.replayInspecting = false;
    state.currentEvidenceKey = "";
    const nextIndex = Math.min(state.replayMaxBatchSeen, getReplayDisplayBatchIndex() + 1);
    state.replayViewBatchIndex = nextIndex;
    state.replayFollowLatest = nextIndex >= state.replayMaxBatchSeen;
    renderReplayFrame({ advance: false });
    return;
  }
  if (shell.dataset.state === "running") {
...
officeStage.addEventListener("click", (event) => {
  ...
  if (state.currentEvidenceKey === key) {
    ...
    if (state.replayTimer) {
      const displayIndex = getReplayDisplayBatchIndex();
      state.replayFollowLatest = displayIndex >= state.replayMaxBatchSeen;
      renderReplayFrame({ advance: false });
    }
    return;
  }
  ...
  if (state.replayTimer) {
    state.replayInspecting = true;
    const selectedBatch = batchIndexFromEvidenceKey(key);
    state.replayViewBatchIndex = selectedBatch;
    state.replayFollowLatest = selectedBatch >= state.replayMaxBatchSeen;
  }
...
function renderTracePanel(key) {
  ...
  const focusedTurns = item.focusedTurns?.length ? item.focusedTurns : turns.filter((turn) =>
    turn.constraint_text && item.constraint && turn.constraint_text === item.constraint
  );
  inspectorBody.innerHTML = `
    ...
    <div class="trace-focus">
      <div class="trace-focus-head">
        <b>关键 Segment${item.selectedSegmentId != null ? ` #${escapeHtml(item.selectedSegmentId)}` : ""}</b>
        <span>${focusedTurns.length || 0} 个命中 turn</span>
      </div>
      <div class="turn-list focus-list">
        ${focusedTurns.map(traceTurnHTML).join("") || '<div class="panel-empty">这条证据没有独立 Segment turn，下面展示完整对话组。</div>'}
      </div>
    </div>
    <div class="trace-context-title">完整对话组上下文</div>
    ${item.openingLine ? `<article class="turn-card opening-turn">
      <header><span>开场</span><small>Agent opening line</small></header>
      <div class="utterance agent">
        <b>Agent</b>
        <p>${escapeHtml(item.openingLine)}</p>
      </div>
    </article>` : ""}
    <div class="turn-list">
      ${turns.map((turn) => traceTurnHTML(turn, item.constraint)).join("") || '<div class="panel-empty">这组对话没有逐轮记录。</div>'}
    </div>
  `;
}

function traceTurnHTML(turn, activeConstraint = "") {
  const isFocus = activeConstraint && turn.constraint_text === activeConstraint;
  return `<article class="turn-card ${isFocus ? "is-focus" : ""}">
    <header>
  ...
  reportBars.innerHTML = summaries.length
    ? summaries.map((item) => {
      ...
      return `<div class="report-bar-row">
        ...
        <span class="report-bar-counts">${Number(item.pass_count) || 0}/${reportBarTotal(item)}</span>
...
function reportBarTotal(item) {
  return Number(item.total_segments) ||
    ((Number(item.pass_count) || 0) + (Number(item.fail_count) || 0) + (Number(item.invalid_count) || 0));
}
...
function selectReportBatch(index) {
  ...
  // 更新批次指示器
  batchIndicator.hidden = batchCount <= 1;
  batchLabel.textContent = `第 ${safeIndex + 1} 批`;
...
function stopWorkspaceReplay(options = {}) {
...
function startWorkspaceReportReplay() {
  if (!state.report) return;
  stopWorkspaceReplay();
  state.replayBatchIndex = 0;
  state.replayTurn = 0;
  state.replayEvalIndex = 0;
  state.replayMaxBatchSeen = 0;
  state.replayViewBatchIndex = 0;
  state.replayFollowLatest = true;
  state.replayInspecting = false;
  ...
  renderSegmentStrip();
  renderReplayFrame();
  state.replayTimer = setInterval(renderReplayFrame, 1000);
}
...
function renderReplayFrame(options = {}) {
  ...
  const size = getReportBatchSize(state.report);
  const batchCount = Math.max(1, Math.ceil(groups.length / size));
  const autoIndex = Math.max(0, Math.min(batchCount - 1, state.replayBatchIndex));
  const displayIndex = state.replayFollowLatest
    ? autoIndex
    : Math.max(0, Math.min(state.replayMaxBatchSeen, state.replayViewBatchIndex));
  state.replayViewBatchIndex = displayIndex;
  const offset = displayIndex * size;
  const batchGroups = groups.slice(offset, offset + size);
  const autoGroups = groups.slice(autoIndex * size, autoIndex * size + size);
  const autoMaxTurns = Math.max(1, ...autoGroups.map((group) => (group.turns || []).length));
  const displayMaxTurns = Math.max(1, ...batchGroups.map((group) => (group.turns || []).length));
  const displayTurn = state.replayFollowLatest ? state.replayTurn : displayMaxTurns + 2;
  state.replayMaxBatchSeen = Math.max(state.replayMaxBatchSeen, autoIndex);

  if (state.replayInspecting) {
    if (state.replayFollowLatest) {
      setStationCount(size);
      renderReplayStations(batchGroups, offset, displayTurn);
      renderReplayBatchControls(batchCount);
      const selected = state.currentEvidenceKey;
      document.querySelector(`[data-popover="${selected}"]`)?.classList.add("is-selected");
      if (selected) previewConversationGroup(selected);
    }
    if (shouldAdvance) advanceReplayCursor(autoMaxTurns, batchCount);
    return;
  }
  ...
  renderReplayBatchControls(batchCount);
  renderBatchSummary(offset, batchGroups.length, {
    title: `第 ${displayIndex + 1} 批`,
    subtitle: `${batchGroups.length} 组对话 · C${offset}–C${offset + batchGroups.length - 1}`,
    hint: state.replayFollowLatest ? "同一批 5 组完成后自动进入下一批" : "历史批次已固定，可切回最后一批继续看实时更新",
  });
  document.querySelector("#runHistory").hidden = true;
  document.querySelector("#runHistory").innerHTML = "";
  document.querySelector("#batchSummary").hidden = false;

  renderReplayEvalStrip(state.replayEvalIndex);

  if (shouldAdvance) advanceReplayCursor(autoMaxTurns, batchCount);
}

function advanceReplayCursor(maxTurns, batchCount) {
  state.replayTurn += 1;
  state.replayEvalIndex = (state.replayEvalIndex + 1) % Math.max(1, state.segments.length);
  if (state.replayTurn > maxTurns + 1) {
    state.replayTurn = 0;
    state.replayBatchIndex = (state.replayBatchIndex + 1) % batchCount;
  }
}

function renderReplayBatchControls(batchCount) {
  batchIndicator.hidden = state.replayMaxBatchSeen <= 0;
  const displayIndex = getReplayDisplayBatchIndex();
  const latestSeen = Math.min(state.replayMaxBatchSeen, batchCount - 1);
  batchLabel.textContent = `第 ${displayIndex + 1} 批`;
  batchPrev.disabled = displayIndex <= 0;
  batchNext.disabled = displayIndex >= latestSeen;
}

function getReplayDisplayBatchIndex() {
  return state.replayFollowLatest ? state.replayBatchIndex : state.replayViewBatchIndex;
}

function batchIndexFromEvidenceKey(key) {
  const match = String(key || "").match(/^c(\d+)$/i);
  if (!match) return getReplayDisplayBatchIndex();
  const size = getReportBatchSize(state.report);
  return Math.floor(Number(match[1]) / size);
}

function renderReplayStations(batchGroups, offset, turnCursor) {
  stationKeys.forEach((key, index) => {
    const group = batchGroups[index];
    const cid = offset + index;
    const evidenceKey = `c${cid}`;
    if (!group) {
      updateStation(key, { title: `C${cid}`, chip: "等待", type: "wait", bubble: "", evidenceKey });
      return;
    }
    const turns = group.turns || [];
    const currentTurn = turns[Math.min(Math.max(turnCursor - 1, 0), Math.max(0, turns.length - 1))];
    let chip = "开场";
    let type = "active";
    if (turnCursor > 0 && currentTurn) {
      chip = `T${currentTurn.local_turn_index || Math.min(turnCursor, turns.length)}`;
    }
    if (turnCursor > turns.length) {
      chip = "完成";
      type = "done";
    }
    state.evidence[evidenceKey] = {
      ...(state.evidence[evidenceKey] || {}),
      statusLabel: chip,
      statusType: type,
      statusDetail: type === "done" ? `完成 ${turns.length} 轮对话。` : `正在进行 ${chip}。`,
      latestUser: currentTurn?.user_reply || state.evidence[evidenceKey]?.latestUser || "--",
      latestAgent: currentTurn?.agent_reply || state.evidence[evidenceKey]?.latestAgent || "--",
    };
    updateStation(key, { title: `C${group.conversation_id ?? cid}`, chip, type, bubble: "", evidenceKey });
  });
}

function renderReplayEvalStrip(activeIndex) {
  if (!state.segments.length) return;
  state.segments.forEach((seg, index) => {
    if (index < activeIndex) seg.status = seg.finalStatus || "pending";
    else if (index === activeIndex) seg.status = "evaluating";
    else seg.status = "pending";
  });
  renderSegmentStrip();
  const active = evalStrip.querySelector('.seg-card[data-status="evaluating"]');
  scrollEvalCardIntoView(active);
}
...
function renderBatchSummary(offset, count, meta = null) {
  ...
  const prefix = meta ? `<div class="batch-replay-head"><b>${escapeHtml(meta.title)}</b><span>${escapeHtml(meta.subtitle)}</span><em>${escapeHtml(meta.hint)}</em></div>` : "";
  groupsRow.innerHTML = prefix + tags;
...
function buildEvidence(report) {
  ...
  (report.conversation_groups || []).forEach((group, index) => {
    ...
    state.evidence[key] = {
      ...
      selectedSegmentId: selectedEval?.segment_id || lastSegment?.segment_id || null,
      focusedTurns: selectedEval?.segment_turns || lastSegment?.turns || [],
      openingLine: group.opening_line || report.opening_line || group.turns?.[0]?.history_before?.find((entry) => entry.role === "assistant")?.content || "",
...
function buildEvidenceItemFromGroup(group, evals = [], fallbackIndex = 0) {
  ...
  return {
    ...
    selectedSegmentId: selectedEval?.segment_id || lastSegment?.segment_id || null,
    focusedTurns: selectedEval?.segment_turns || lastSegment?.turns || [],
    openingLine: group.opening_line || group.turns?.[0]?.history_before?.find((entry) => entry.role === "assistant")?.content || "",
    conversationId: id,
    turns: group.turns || [],
    segments,
    evaluations: groupEvals,
...
function buildSegmentsFromReport(report) {
  ...
  return (report.segments || []).map((seg) => {
    ...
    return {
      ...
      finalStatus: ev ? ev.result : "pending",
...
function highlightSegCard() {
  ...
  scrollEvalCardIntoView(el);
}

function scrollEvalCardIntoView(card) {
  if (!card) return;
  const stripRect = evalStrip.getBoundingClientRect();
  const cardRect = card.getBoundingClientRect();
  const targetLeft = evalStrip.scrollLeft +
...
function renderLiveBatchControls() {
  ...
  batchLabel.textContent = `第 ${state.activeBatch} 批`;
...
async function startRun() {
  stopWorkspaceReplay({ restore: true });
