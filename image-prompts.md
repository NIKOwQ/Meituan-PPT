...
> 所有图片共享风格：扁平化矢量插画，美团黄 #FFD100 + 深灰 #333333，白色背景，干净线条
...
Style: flat vector, minimal, clean geometric lines, no gradients, no 3D, no characters, no photos. Color palette: #FFD100 (gear and accent lines), #333333 (document, report, primary lines), #FF4D4F (one red line in report), #52C41A (one green line in report), white background. Professional and modern.

Strictly flat 2D vector style. No realistic rendering. Think of illustrations in Stripe's website or Linear's marketing pages.
```
...
```
Create a flat vector illustration in 16:9 aspect ratio showing a web application dashboard screenshot mockup.

A browser window frame (rounded corners, three dots in title bar) containing:
- Left panel (30% width): A document viewer showing highlighted text lines, with a coverage heatmap overlay using #FFD100 (high coverage) and #FF4D4F (low coverage) colored sidebar indicators.
- Right panel (70% width): 4 small worker cards arranged in a 2x2 grid, each showing a chat conversation preview (alternating message bubbles). SVG animated lines connect the cards. One card has a red (#FF4D4F) badge, others have green (#52C41A) badges.
- Bottom: A progress bar at 64% filled with #FFD100.

Style: flat vector, UI mockup style, clean lines, no screenshots, no real text. Color palette: #333333 (borders, text), #FFD100 (progress, highlights), #FF4D4F (alerts), #52C41A (success), #FAFAFA (panel backgrounds), white outer background.
